...
SELECT
    ...
    RANK() OVER (
        ORDER BY SUM(s.total_sale_value) DESC
    ) AS revenue_rank
FROM buyers b
JOIN sales s
...
SELECT
    ...
    total_procured_kg,
    RANK() OVER (
        ORDER BY total_procured_kg DESC
    ) AS procurement_rank
...
warehouse_efficiency AS (
    SELECT
        w.warehouse_id,
        w.warehouse_name,
        w.capacity_kg,
        COALESCE(wi.current_inventory_kg, 0) AS current_inventory_kg,
        ROUND(
            COALESCE(wi.current_inventory_kg, 0) * 100.0
