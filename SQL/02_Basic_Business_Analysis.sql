...
SELECT
    ...
    ROUND(100.0 * SUM(c.collected_quantity_kg)/ SUM(SUM(c.collected_quantity_kg)) OVER (),2) AS procurement_share_pct
FROM forests f
JOIN collection c
...
WITH forest_revenue AS (SELECT c.forest_id, SUM(s.total_sale_value) AS revenue_inr
    ...
    GROUP BY c.forest_id),forest_expense AS (SELECT c.forest_id,SUM(e.amount_inr) AS expense_inr
    FROM expenses e
    JOIN contracts c
...
SELECT
    ...
    COALESCE(r.revenue_inr, 0)- COALESCE(e.expense_inr, 0) AS operating_profit_inr
FROM forests f
